...
const Slider = () => {
  ...
  const createLineElements = async (element) => {
  ...
  const processTextElements = async (container) => {
    ...
    for (const p of paragraphs) {
      await createLineElements(p);
    }
  };

  const createSlideElement = (slideData) => {
    const content = document.createElement("div");
    content.className = "slider-content opacity-0";
    content.innerHTML = `
      <div class="slide-title text-center mt-10">
        <h1 class="flex justify-center gap-[0.2em] text-[60px]">${slideData.title}</h1>
      </div>
      <div class="slide-description px-10 mt-4">
        <p>${slideData.description}</p>
        <div class="slide-info uppercase mt-2">
  ...
  const setupInitialSlide = async () => {
    ...
    const slideElement = createSlideElement(slides[currentSlideIndex]);
    container.appendChild(slideElement);

    await processTextElements(slideElement);

    const chars = slideElement.querySelectorAll(".char span");
    const lines = slideElement.querySelectorAll(".line span");
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      interval = setInterval(handleSlideChange, 5000); // 自動輪播
  ...
  return (
    <div
      className="relative w-full h-[700px] text-white overflow-hidden"
      ref={sliderRef}
    >
